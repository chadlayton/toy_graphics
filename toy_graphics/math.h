...
namespace math
{
		...
		const vec<3>& up)
	{
		...
		const vec<3> right = cross(up, forward);
		return{
			vec<4>(right.x, up.x, forward.x, 0),
			vec<4>(right.y, up.y, forward.y, 0),
			vec<4>(right.z, up.z, forward.z, 0),
	...
	template<typename T>
	T saturate(T val, T min = 0, T max = 1)
	{
